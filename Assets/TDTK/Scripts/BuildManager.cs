    ...
    public class BuildManager : MonoBehaviour
    {
        ...
        public BuildInfo _CheckBuildPoint(Vector3 pointer, int towerID)
        {
            ...
            if (mainCam != null)
            {
                ...
                if (Physics.Raycast(ray, out hit, Mathf.Infinity, maskPlatform))
                {
                    Debug.Log("click mouse and build tower");
                    for (int i = 0; i < buildPlatforms.Count; i++)
                    {
        ...
        {
            Debug.Log($"Build One Tower When Mouse Click Menu");
            Debug.Log($"BuildTower({ID}, {bInfo.status} {bInfo.position} {bInfo.platform.name}");
        ...
        {   //called from UnitTower.DragNDropRoutine
            if (bInfo == null)
            {
